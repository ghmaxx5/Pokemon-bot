...
async function execute(message, args, spawns, prefix) {
  ...
  if (subcommand === "list" || subcommand === "add") {
    ...
    if (isNaN(position) || isNaN(price) || price < 1) {
      return message.reply("Invalid Pokemon position number or price.");
    }
...
module.exports = { name: "market", aliases: ["m"], description: "Buy and sell Pokemon on the market", execute };
